# REGICIDE PVP — Handoff v128.4 (August 2026)

Read this at the start of a new chat to continue. `claude/REGICIDE-DEPLOY.md` covers hosting.
Supersedes `REGICIDE-HANDOFF-v128.3.md`, which can be deleted from the project.

**On disk: `v128.4 — THE LANE THAT COULD NOT DROP`.** Next version: **v128.5** (or v129 if the
unit-merge below gets built, because that is a big enough change to earn a number).

> ### ⚠ PROTO IS STILL 26.
> v128.4 put three new fields on the wire and bumped nothing, because all three are OPTIONAL
> and additive: `aq` and `et` are guest→host (an old host ignores them), and `bye` is a new
> message type that `hostData` already falls through on. Nothing existing changed shape.
...
1. **Re-run the field test on v128.4.** Same drill: host + guests, 15+ minutes, F9 on every
   machine, `node tools/netlog.js <host.json> <guest…>`. What to check now:
   ...
   | section 3 `guest got` vs `host sent` | now comparable — it was reading 171% because relay sends were uncounted |
   | `drop` events | **one per departure**, with a reason: `left` / `pc-failed` / `silent-Ns` |
   | guest `stale` | should collapse with the floods gone |

2. **If outages persist but are short**, the fast lane is still dying too often — that is item 4
   and 5 of the old table (15 redials, 7 wedges) and the next dial to turn is `LANE_WEDGE_MS`,
   which is currently doing harm as well as good.
3. **Then the unit rigid-cluster merge**, below. `simR` is 0.779 and the host is at 17 fps.
...
`js/10-net.js`, ~2050 lines. PeerJS, **host-authoritative**. Constants at lines 26–80:
...
- **PROTO is 26.** Bump it only for a change an old peer would *misread*. An OPTIONAL field an old
  ...
  v128.4's `aq`, `et` and `bye` shipped. **OMITTING** a field an old peer reads unguarded is the
  other case, and that is v127's envelope delta: 25 → 26.
...
|---|---|---|---|---|
| `node tools/smoketest.js` | **405 PASS** (was 388 + 17 new) | ~100 s | — | anything |
...
The 17 new assertions cover the relay rate cap, the ack window opening and releasing, the mirror's
exemption, `sentR` counting, the reaper's 90 s threshold *and* its refusal to fire at 25 s, `bye`,
`hostDrop` back-compatibility, and the e-tap — including a test that **reproduces the pinned-bit
bug first**, so it cannot pass vacuously.
...
- **THE v128.4 FIELD TEST.** Top of this document. Everything netcode is downstream of it.
