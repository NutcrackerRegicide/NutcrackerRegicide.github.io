# REGICIDE PVP — Handoff v123 (July 2026)

Upload this file at the start of a new chat (or read it from the "Nutcracker Regicide" project) to continue development. Supersedes all prior handoffs. Covers **v107 THE SCORE & THE 90-SECOND ADVANCE** (recap), **v108 THE AUDIT** (recap), **v109 THE MIX FIX & THE VOICES**, **v110 THE WOLVES**, **v111 THE QUIET FIXES**, **v112 WOLF QUIET + NEW ANTHEMS**, **v113 THE FIELD PASS**, **v114 THE GREAT WOOD**, **v115 THE STANDS**, **v116 THE MOBILE SPIKE**, **v117 TWIN STICK**, **v118 NO WALLS**, **v119 THE ROTATED STAGE**, **v120 THE MOBILE GAME**, **v121 THE HOME-SCREEN PASS**, **v122 THE LEAK**, and this build: **v123 THE ANTHEM DIET**.

> **PROTO 23 → 24 → 25.** Neither build changes a message format, but both change the generated world, and `nodes` rides the wire as a bare array indexed by generation order. Two clients with different forests agree on the bytes and disagree about which tree index 300 is, so the handshake has to reject the mismatch. **Everyone must run v123.** (v116-v123 are client-side only — PROTO stays 25.) (Rule worth keeping: *any* change to world generation is a PROTO bump, not just a change to the wire format.)

## v123 THE ANTHEM DIET — and the deploy trap is gone

John ran out of Netlify credits. The free plan is **300 credits a month, reset at the start of each
billing cycle, hard-capped** — when they're gone every project is paused and shows "Site not
available." The cause wasn't the game: **the anthems were 49 MB of a 52 MB site**, and every fresh
phone load streams one.

### Measured, not guessed
A 2-minute excerpt of "Stone Age" encoded every plausible way, each result's spectrum compared
against the source (age0, which shipped at 4.86 MB / 105 kbps stereo q3):

| encode | size | kbps | mean spectral error | HF kept |
|---|---|---|---|---|
| stereo q3 *(what shipped)* | 4.86 MB | 105 | 10.02 dB | 111% |
| stereo q1 | 3.45 MB | 74 | 12.31 dB | 107% |
| stereo q0 | 2.73 MB | 59 | **16.35 dB** | 93% |
| mono q3 | 3.09 MB | 66 | **9.72 dB** | 105% |
| **mono q1 ← shipped** | **2.72 MB** | **59** | **11.71 dB** | 101% |
| mono q0 | 2.17 MB | 47 | 15.37 dB | 90% |

The useful finding: **at a given file size mono beats stereo comfortably** — mono q1 and stereo q0
are the same 2.7 MB, but mono has 4.6 dB less spectral error. Every bit goes into one channel.
Mono q3 is even *better than what shipped* while still being 36% smaller.

**The caveat, stated plainly:** that metric downmixes to mono before comparing, so it is blind to
stereo-image loss *by construction*. It is the right metric here — the anthems are a background bed
ducked to 0.33 under the SFX bus and trimmed again by `MUSTRIM` 0.42, and a phone speaker is mono —
but if a future track deserves stereo on desktop headphones, run `MUSIC_AC=2 MUSIC_Q=1` and accept
~36 MB instead of ~26 MB.

**Result: 49.1 MB → 26.2 MB, 47% smaller**, all six durations bit-identical (390/682/610/598/718/657 s).
Loudness went from −14.0/−13.5/−12.7 LUFS to −14.6/−15.0/−14.7 — marginally quieter *and tighter
between tracks*, so `MUSTRIM` 0.42 stands unchanged.

### THE ZIP NOW CARRIES THE MUSIC — the v111 deploy trap is dead
The full build is **29.14 MB**, under the 30 MB delivery cap. Since v108 the zip has excluded
`audio/music/`, which is why a fresh zip extract deployed silently with no anthems and no error
anywhere — the bug that cost a whole version to diagnose. **That is over.** Zip, extract, deploy;
the music comes with it.

### `tools/music.sh` — because the songs are being replaced
The whole point. Edit the six-line age→filename map at the top and run it; it converts, reports
before/after per track, and reminds you to run the browser check. `--dry` previews, and
`MUSIC_Q` / `MUSIC_AC` / `MUSIC_AR` override the encode. The measured table above lives in a
comment at the top of the file so the next person doesn't have to re-derive it.

`browsercheck` gained a **music budget** guard — the six anthems must total under 32 MB. That is
there specifically to stop the next song swap silently putting the site back to 50 MB.

### Hosting, since Netlify is capped
Netlify's credits *do* reset each cycle, so the existing sites come back on their own.
[**Cloudflare Pages**](https://developers.cloudflare.com/pages/platform/limits/) is the closest
replacement — same drag-a-folder flow, free, 20,000 files (this project has ~175) and 25 MiB per
file (the largest anthem is now 5.2 MB), and it gives a real domain so **Add to Home Screen keeps
working**. [itch.io](https://itch.io/docs/creators/html5) is a good shopfront (500 MB, 1,000 files,
free) but serves games in an iframe, so the PWA fullscreen and rotated stage won't behave the same.
...
## Transcripts
Carry-forward: project doc `claude/REGICIDE-HANDOFF-v123.md`, device `docs/REGICIDE-HANDOFF-v123.md`. Delete older handoffs from the project.
