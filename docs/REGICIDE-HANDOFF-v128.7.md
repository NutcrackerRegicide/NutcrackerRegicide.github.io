# REGICIDE PVP — Handoff v128.7 (August 2026)

Read this at the start of a new chat to continue. `claude/REGICIDE-DEPLOY.md` covers hosting.
Supersedes `REGICIDE-HANDOFF-v128.6.md`, which can be deleted from the project.
Two companion docs, both still current:
`claude/REGICIDE-NETCODE-GAP-ANALYSIS.md` — what this netcode does and does not have, audited.
`claude/REGICIDE-TOPOLOGY.md` — P2P vs dedicated vs hybrid, with the headless tick-cost number.

**On disk: `v128.7 — THE RELOAD THAT TAKES`.** Next version: **v128.8**.
...
# ⇢ WHAT v128.7 WAS: MAKING A DEPLOY ACTUALLY ARRIVE

**Symptom:** John pushed v128.6, his phone showed v128.6, and his desktop kept showing v128.5.
The live site was confirmed serving v128.6, so this was purely client-side. Third deploy-freshness
incident in six versions — and this file has now caused all three.

**Cause, and it is subtle.** `sw.js`'s network-first branch called a bare `fetch(req)`. A bare
fetch uses the DEFAULT cache mode, which consults the browser's own HTTP cache and may answer from
it **without ever reaching the origin** while the response is still fresh by `max-age` — and
GitHub Pages serves HTML with one. So the worker faithfully asked for `index.html`, the browser
answered out of its own cache, and the page reported a build that had been superseded.

v128.3's comment already describes the intended behaviour — *"GitHub Pages sends ETags, so in the
common case this is a conditional request that comes back 304"* — but a plain `fetch()` never
guarantees the request is made at all. **`fetch(req,{cache:"no-cache"})` does**: always revalidate,
still take the cheap 304. That is the whole fix for the desktop symptom.

**Three more things went in with it:**

1. **The second load is automated.** `skipWaiting` + `clients.claim` mean a new worker takes over
   the instant it installs — but the page on screen was served by the OLD one, which is why the
   rule was ever "reload twice". A `controllerchange` listener now reloads once, guarded three
   ways: not on a first visit (there was no previous controller), not twice (a latch), and
   **never mid-match** — there is no host migration, so a host reloading ends the game for
   everyone in it. The mid-match case deliberately does *not* latch, so it takes the next chance.
   `inMenu` is a top-level `let`, so reading it from the `<head>` script is a temporal-dead-zone
   hazard where even `typeof` throws — hence the try/catch.
2. **`updateViaCache:"none"`** on the registration, plus a `reg.update()` on every load, so the
   worker script itself can never be answered from the HTTP cache.
3. **`js/audio-data.js` moved to the cache-first branch.** It is 3.9 MB of base64 audio that
   changes about once a year, and it was in `js/` — so `isCode` swept it into network-first and a
   phone revalidated it on every single load. By the file's own stated policy it belongs with
   `libs/` and `assets/`, "the megabytes that only ever change on a real version bump".

## Nine new assertions, because none of this is visible from inside the game

The one that matters most: **`sw.js`'s `VERSION` must equal the menu verstamp.** The v128.2
disaster was an unbumped `VERSION`, and nothing in the codebase could see it. The others assert
the network-first branch revalidates, the cache-first branch does *not* (that half must stay
cheap), the registration options, all three reload guards, that the audio blob is on the cheap
path, and that **every `SHELL` entry resolves to a real file** — `install()` deliberately swallows
a 404 per entry so one bad path cannot disable the worker, which also means a path that drifts out
of step with the repo silently stops being precached for ever, with no symptom but a slow launch.

---

# ⇢ WHAT v128.6 WAS: THE UNIT MERGE — 51 MESHES TO 11, 26 MATERIALS TO 1

The largest measured lever in the project, and it is spent. Measured with `tools/drawcost.js` and
`tools/perfcheck.js` at the same base-fight vantage point:

| | before | after |
|---|---|---|
| draw calls, whole scene | **1,516** | **675** |
| draw calls per unit | 22.8 | **7.4** |
| `perfcheck` base fight | 1,497 calls · 25.2 ms | **671 calls · 15.8 ms** |
| broadsword body | 51 meshes / 26 materials | **11 meshes / 1 material** |
| knight (mounted) | 115 / 62 | **21 / 2** |
| scene geometries (vista) | 3,279 | **1,386** |

**2.25x fewer draw calls for the whole scene, 3.1x fewer per unit, and 37% off the frame at the
busiest vantage point.** `tools/unitshot.js` (new) renders one unit against a fixed camera with the
world hidden — the before/after pair is pixel-identical, including the horse rig.

## Two corrections to what the old handoff promised

1. **It is ELEVEN clusters, not seven.** `animateUnit` writes `R.shinL/shinR` (knees) and
   `R.faL/faR` (elbows) independently of the thigh and upper arm on every frame, so a leg is two
   clusters and an arm is two. Eleven is the floor without moving to skinning. Mounted units are
   21 because the horse's four legs and four knees animate too.
2. **Merging geometry alone would not have worked.** A merged cluster still costs one draw per
   material it contains, and `mat()` (`01-engine.js:109`) caches nothing — every `box()`/`cyl()`/
   `cone()` mints a fresh `MeshToonMaterial`. A broadsword's 51 meshes carried **26** materials, so
   a pure cluster merge lands at **41** draws, not 11. The materials had to collapse too.

## Why an atlas and not vertex colours

The old handoff left this open and called it "the whole design". It is, and the evidence is
one-sided: **25 of a broadsword's 51 meshes are textured**, and they are every large surface —
torso, both arms, pelvis, both thighs, all four armour rows, helm, head, shield. That is not a
minority you can absorb. The `uniform` swatch has 1-texel gold button columns; the torso cylinder
is 64 vertices. The 64x64 face has 3x4 pupils and 2-texel teeth on a 100-vertex lathe. Reproducing
those per-vertex means tessellating, which spends back in vertices exactly what the merge saves in
draws.

The atlas, by contrast, was nearly free: every unit texture is **16x16 or 64x64** (power of two),
**ClampToEdge**, **NearestFilter**, **no mipmaps**, and **nothing on a unit tiles** — the only
`RepeatWrapping` texture in the game is the terrain grass. The all-classes-all-ages ceiling is 130
textures / 110,080 texels. `UATLAS` (`01-engine.js`) is 1024² with a skyline allocator, and **a
cell never moves once allocated**, because merged geometry has its UVs baked in.

**One material carries both channels.** Verified in the shipped `libs/three.min.js`: `meshtoon_frag`
runs `<map_fragment>` then `<color_fragment>` and both multiply into `diffuseColor`, so
`MeshToonMaterial({map:ATLAS, vertexColors:true, gradientMap:TOON_RAMP})` composes atlas × vertex
colour × toon ramp correctly. A textured part gets an atlas cell and white vertex colour; a flat
part gets its hex baked to vertex colour and points at a white cell.

## Three details that will bite anyone who touches this

1. **UVs are clamped to [0,1] before they are mapped into a cell.** r128's `SphereGeometry` emits
   UVs out to ±8.3% at the poles — ~1.3 texels on a 16×16 skin — which without the clamp would
   sample a *neighbouring* skin in the atlas. Every unit texture is ClampToEdge today, so clamping
   reproduces the exact pixel it samples now. The atlas also edge-extends each cell into 2px of
   padding for the same reason.
2. **The merge disposes the source materials, and it must.** `_skinCache` materials are shared and
   are left alone (`isSharedMat`), but the ~10 per broadsword and ~20 per age-5 villager minted by
   the uncached `mat()` were being orphaned on *every* rebuild — a leak the v122 geometry fix never
   covered. `restyleUnits` rebuilds every villager on the team on every age-up.
3. **Merged geometry is per-unit and must never be cached per class.** The v122 disposer walks
   `u.body` and frees anything with a `.geometry`; a shared template would be freed the first time
   any unit of that class rebuilt, and every other unit of that class would turn to garbage. This
   is the same hazard as `MODELS[cls]`, and the guard is the same one.

## What is excluded from the merge, and why

`_mergeableMat` only folds **plain toon materials** — not transparent, not `flatShading`, front
side. That automatically leaves out the priest's transparent `MeshBasicMaterial` aura, any ink
hull's `ShaderMaterial` and the name-tag sprite, **without naming any of them**, so it stays
correct when someone adds another. `R.musketG` and `R.bowG` are cluster roots of their own (they
rotate on aim); `R.logs` is barred entirely because `updateCargoVisual` toggles its children
individually.

## The trade, stated honestly

Triangles per unit went **741 → 1,004**. Two causes: `toNonIndexed()` expansion, and a merged
cluster being frustum-culled as one lump where 11 small meshes were culled individually. Triangles
are cheap and draw calls are not, so this is the right side of the trade — but it is a real cost
and `drawcost.js` reports both numbers for a reason.

## Still on the table for a future pass

- **One draw call per unit** needs skinning: a single `SkinnedMesh` with the 11 rig nodes as bones.
  `animateUnit` writes `Object3D.rotation` on those exact nodes today, so the port is mechanical —
  but it is a real project and 11 is already 3.1x.
- **`TREE_MAT` is a `MeshLambertMaterial` with no `gradientMap`** (`02-world.js`). Every tree in the
  game is lit by a different shading model from everything else in it. Nobody has noticed, which is
  its own kind of answer, but it is a one-line experiment.
- **The tree inside-out shell** the v128 outline work wanted baked into `_mergeColored`. The merge
  machinery now exists twice; if that gets built, unify them.
...
1. **Re-run the field test on v128.5.** Same drill: host + guests, 15+ minutes, F9 on every
   ...
   | check | expect |
   |---|---|
   | frame rate, on any device | **this is the one to ask about first — v128.6 cut the scene's draw calls by 2.25x** |
...
|---|---|---|---|---|
| `node tools/smoketest.js` | **445 PASS** (388 baseline + 17 v128.4 + 21 v128.5 + 9 v128.6 + 10 v128.7) | ~100 s | — | anything |
...
- **THE HARNESS IS PROBABILISTIC, and it is one root cause.** `Math.random` is unseeded, so the
  400 warm-up ticks leave a different world every run. THREE different tests flaked this session
  from that alone: the militia pair, the mobilecheck feed banner, and `the pack savages an intruder
  standing in the camp`. Seeding it would make the whole suite reproducible — but expect to
  re-baseline, because world gen and the AI would both land somewhere new.
...
| `tools/drawcost.js` | A/Bs units in and out of a rendered frame. **7.4 calls per unit as of v128.6, was 22.8.** |
| `tools/unitshot.js` | **new.** ONE unit, fixed camera, world hidden — the only way to eyeball a body change without the AI having rebuilt the town underneath you. |
| `tools/headless.js` | **new.** What a tick costs with no screen: **2.56 ms at 136 units, 5.7x headroom at 30Hz.** The number the dedicated-server question turns on. |
...
- **`audio/` is absent from the sandbox too.** Without it `browsercheck.js` false-reds three
  assertions about the six anthems (`decode/load error`, `0.0 MB of 32`); the 139 SFX still decode
  from `js/audio-data.js` and the http + `file://` boot checks are still valid.
...
- **~~The unit rigid-cluster merge~~ — DONE in v128.6.** 1,516 → 675 draw calls. The next lever of
  that size is skinning (11 → 1 per unit) and it is a much bigger project for a smaller win.
