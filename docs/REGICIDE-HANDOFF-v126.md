# REGICIDE PVP — Handoff v126 (July 2026)

Read this at the start of a new chat to continue. `claude/REGICIDE-DEPLOY.md` covers hosting.
Supersedes `REGICIDE-HANDOFF-v125.1.md`, which should be deleted from the project.

**Shipped: `v126 — THE HONEST CLOCK`.** Next version: **v127**.

> **PROTO stays 25.** v126 adds ONE optional snapshot field (`ht`, the host's own monotonic
> clock in ms). A v125 peer ignores it and falls back to the v95 comparison, which is still in
> the code and still tested. World gen untouched since v115. Layout untouched since v125.1.
...
**Every number in this document is measured from those three files.** Nothing here is estimated.
...
- **PROTO is 25.** Bump it only for a change an old peer would *misread*. An OPTIONAL field an
...
```
smoketest      ALL PASSED   exit 0                                       (~100 s)
...
- **v126 FIELD TEST — three logs again, then `tools/netlog.js`.** See the top of this document.
